...
> ⚠️ **这一步非常重要！** 如果不创建 `unattend.xml`，Sysprep 后首次启动会进入 OOBE 初始化向导，导致 Administrator 密码失效，远程桌面无法连接。

创建文件 `C:\Windows\System32\Sysprep\unattend.xml`，内容见 [docs/unattend.xml](../unattend.xml)，或复制以下 PowerShell 命令：
...
"@ | Out-File -FilePath "C:\Windows\System32\Sysprep\unattend.xml" -Encoding UTF8
```

该文件的作用：
- `SkipMachineOOBE` / `SkipUserOOBE`：跳过首次开机设置向导
- `fDenyTSConnections=false`：启用远程桌面
- `UserAuthentication=0`：允许非 NLA 连接（兼容 Guacamole）
- `FirewallGroup`：自动放行 RDP 3389 端口
...
| 7 | 配置防火墙（放行 RDP 3389） | ☐ |
| 8 | 启用远程桌面 | ☐ |
| 9 | 创建 unattend.xml（跳过 OOBE） | ☐ |
| 10 | 执行 Sysprep /unattend:unattend.xml | ☐ |
| 11 | 创建自定义镜像 | ☐ |
| 12 | 记录 Image ID | ☐ |
